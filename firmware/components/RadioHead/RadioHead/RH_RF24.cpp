...
bool RH_RF24::init()
{
    ...
    uint8_t pkt_len[] = { 0x02 | (1<<4) | (1<<5), 0x01, 0x00 };
    set_properties(RH_RF24_PROPERTY_PKT_LEN, pkt_len, sizeof(pkt_len));

    uint8_t pkt_field1[] = { 0x00, 0x02, 0x00, RH_RF24_FIELD_CONFIG_CRC_START | RH_RF24_FIELD_CONFIG_SEND_CRC | RH_RF24_FIELD_CONFIG_CHECK_CRC | RH_RF24_FIELD_CONFIG_CRC_ENABLE };
    set_properties(RH_RF24_PROPERTY_PKT_FIELD_1_LENGTH_12_8, pkt_field1, sizeof(pkt_field1));

    uint8_t pkt_field2[] = { sizeof(_buf) >> 8, sizeof(_buf) & 0xFF, 0x00, RH_RF24_FIELD_CONFIG_CRC_START | RH_RF24_FIELD_CONFIG_SEND_CRC | RH_RF24_FIELD_CONFIG_CHECK_CRC | RH_RF24_FIELD_CONFIG_CRC_ENABLE };
	...
	uint8_t gpio_config[] = { RH_RF24_GPIO_NO_CHANGE, RH_RF24_GPIO_NO_CHANGE, RH_RF24_GPIO_RX_STATE, RH_RF24_GPIO_TX_STATE };
...
void RH_RF24::handleInterrupt()
{
    ...
    //Serial.printf("interrupt status: %02x %02x %02x %02x %02x %02x %02x %02x", status[0], status[1], status[2], status[3], status[4], status[5], status[6], status[7]);
...
//    if (status[0] & RH_RF24_INT_STATUS_CHIP_INT_STATUS)
    ...
    {
    if (status[4] & RH_RF24_INT_STATUS_PREAMBLE_DETECT)
    {
    }
...
bool RH_RF24::recvRaw(uint8_t* buf, uint16_t* len) {
    if (!available())
    return false;
...
bool RH_RF24::send(const uint8_t* data, uint8_t len)
{
    ...
    _buf[0] = (len + RH_RF24_HEADER_LEN) >> 8;
    _buf[1] = (len + RH_RF24_HEADER_LEN) & 0xFF;
    // Now the rest of the payload in variable length field 2
    // First the headers
    _buf[2] = _txHeaderTo;
    _buf[3] = _txHeaderFrom;
    _buf[4] = _txHeaderId;
    _buf[5] = _txHeaderFlags;
    // Then the message
    memcpy(_buf + 2 + RH_RF24_HEADER_LEN, data, len);
    _bufLen = len + 2 + RH_RF24_HEADER_LEN;
    _txBufSentIndex = 0;
...
bool RH_RF24::sendRaw(const uint8_t* data, uint16_t len)
{
    if (len > RH_RF24_MAX_PAYLOAD_RAW_LEN - 2)
	return false;
	...
	return false;  // Check channel activity*/
    setModeIdle();
    ESP_LOGI("rf24", "setModeIdle"); 
    _buf[0] = len >> 8;
    _buf[1] = len & 0xFF;
    memcpy(_buf + 2, data, len);
    _bufLen = len + 2;
    _txBufSentIndex = 0;

    // Set the field 2 length to the variable payload length
    uint8_t l[] = { (uint8_t)(len >> 8), (uint8_t)(len & 0xFF) };
    set_properties(RH_RF24_PROPERTY_PKT_FIELD_2_LENGTH_12_8, l, sizeof(l));

    sendNextFragment();
    ESP_LOGI("rf24", "sendNextFragment"); 
    setModeTx();
    ESP_LOGI("rf24", "setModeTx"); 
    return true;
}
...
void RH_RF24::sendNextFragment()
{
    ...
    ESP_LOGI("rf24", "sendNextFragment: %d %d %d", len, _bufLen, _txBufSentIndex);
...
void RH_RF24::setModeRx()
{
	...
	uint8_t l[] = { sizeof(_buf) >> 8, sizeof(_buf) & 0xFF };
	set_properties(RH_RF24_PROPERTY_PKT_FIELD_2_LENGTH_12_8, l, sizeof(l));
